...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
    ...
    uint32_t frameLen = 0;          //帧长度
    uint32_t bufOffset = 0;         //数据偏移量
    uint32_t bytesLen = 0;          //要发送的数据长度
    uint32_t payloadoffset = 0;     //数据包中数据的偏移量
    ...
    for(q = p; q != NULL; q = q->next)
    {
        ...
        while((bytesLen + bufOffset)>ETH_TX_BUF_SIZE )
        {
            ...
            bytesLen = bytesLen - (ETH_TX_BUF_SIZE - bufOffset);
            payloadoffset = payloadoffset + (ETH_TX_BUF_SIZE - bufOffset);
            frameLen = frameLen + (ETH_TX_BUF_SIZE - bufOffset);
            bufOffset = 0;
        }
        //拷贝剩余的数据
        memcpy( (uint8_t*)((uint8_t*)buffer + bufOffset),\
                (uint8_t*)((uint8_t*)q->payload + payloadoffset), bytesLen);
        bufOffset = bufOffset + bytesLen;
        frameLen = frameLen + bytesLen;
    }
    //当所有要发送的数据都放进发送描述符的Tx Buffer以后就可发送此帧了
    HAL_ETH_TransmitFrame(&ETH_Handler, frameLen);
    errval = ERR_OK;
error:    
...
{  
  ...
  uint32_t byteslefttocopy=0; //需要拷贝的字节长度
  uint32_t i=0;
