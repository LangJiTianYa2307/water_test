...
{      
  ...
  HAL_GPIO_Init(GPIOI,&GPIO_Initure);     //初始化GPIOB.0和GPIOB.1

  
  INTX_DISABLE();                         //关闭所有中断，复位过程不能被打断！
	HAL_GPIO_WritePin(GPIOI,GPIO_PIN_8,GPIO_PIN_RESET);	//PB1置1
  delay_xms(100);
	HAL_GPIO_WritePin(GPIOI,GPIO_PIN_8,GPIO_PIN_SET);	//PB1置1
  delay_xms(100);
  INTX_ENABLE();                          //开启所有中断  
...
void HAL_ETH_MspInit(ETH_HandleTypeDef *heth)
{
    ...
    HAL_NVIC_SetPriority(ETH_IRQn,5,0);         //网络中断优先级应该高一点,这个是FREE所能管理的最高优先级
...
{ 
	DMARxDscrTab = myMalloc(MEM_SRAM, ETH_RXBUFNB * sizeof(ETH_DMADescTypeDef));//申请内存
	DMATxDscrTab = myMalloc(MEM_SRAM, ETH_TXBUFNB * sizeof(ETH_DMADescTypeDef));//申请内存  
	Rx_Buff = myMalloc(MEM_SRAM, ETH_RX_BUF_SIZE * ETH_RXBUFNB);	//申请内存
	Tx_Buff = myMalloc(MEM_SRAM, ETH_TX_BUF_SIZE * ETH_TXBUFNB);	//申请内存
...
{ 
	myFree(MEM_SRAM,DMARxDscrTab);  //释放内存
	myFree(MEM_SRAM,DMATxDscrTab);  //释放内存
	myFree(MEM_SRAM,Rx_Buff);		    //释放内存
	myFree(MEM_SRAM,Tx_Buff);		    //释放内存  
}
